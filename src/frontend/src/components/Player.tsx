...
export const Player = () => {
  ...
  const { id: userId, currentSong, queue, queueIndex, isShuffled } =
  ...
  const handleSongEnd = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.usuario) {
          const state = (await import("../store")).store.getState().gamification;
    ...
    const currentState = (await import("../store")).store.getState().gamification;
    if (currentState.queue.length > 0 && currentState.queueIndex < currentState.queue.length - 1) {
    ...
    <div className="fixed bottom-0 w-full bg-zinc-900 border-t border-zinc-800 z-50">
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex items-center gap-3 w-[30%] min-w-0">
          ...
          {showXpGained && (
            <div className="ml-2 bg-green-500/20 text-green-400 text-xs font-bold px-2 py-1 rounded-full animate-pulse whitespace-nowrap">+{lastXpGained} XP</div>
          )}
        </div>

        <div className="flex items-center gap-4">
          <button className="text-gray-400 hover:text-white transition text-lg">⏮</button>
          <button onClick={togglePlay} className="bg-white text-black w-9 h-9 rounded-full flex items-center justify-center hover:scale-105 transition shadow-lg">
            {isPlaying ? "⏸" : "▶"}
          </button>
          <button onClick={() => dispatch(nextInQueue())} className="text-gray-400 hover:text-white transition text-lg">⏭</button>
        </div>

        <div className="w-[30%] flex justify-end items-center gap-3">
          {isShuffled && <span className="text-xs text-green-400 font-bold">🔀</span>}
