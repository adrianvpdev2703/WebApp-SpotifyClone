...
export const Header = () => {
  ...
  const xpProgress = user.xpRequerida > 0 ? (user.xp / user.xpRequerida) * 100 : 0;
    ...
    <header className="bg-black text-white px-6 py-3 flex justify-between items-center border-b border-zinc-800 sticky top-0 z-50">
      <div className="flex items-center gap-6">
        ...
        <nav className="hidden md:flex items-center gap-4">
          <Link to="/" className="text-gray-300 hover:text-white font-semibold text-sm transition">
            Inicio
          </Link>
          <Link to="/ranking" className="text-gray-300 hover:text-white font-semibold text-sm transition">
            Ranking
          </Link>
          <Link to="/tienda" className="text-gray-300 hover:text-white font-semibold text-sm transition">
            Tienda
          </Link>
          <Link to="/admin" className="text-gray-300 hover:text-white font-semibold text-sm transition">
            Admin
          </Link>
        </nav>
      </div>
      ...
      {user.id ? (
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-2 bg-zinc-900 px-4 py-2 rounded-full border border-zinc-700">
            <div className="flex flex-col">
              <div className="flex items-center gap-3 text-xs">
                <span className="text-green-500 font-bold">Lv.{user.nivel}</span>
                <span className="text-orange-400">🔥 {user.racha}d</span>
                <span className="text-yellow-400">💎 {user.puntos}</span>
              </div>
          ...
          <button
            onClick={() => useGamificationStore.getState().clearUser()}
        ...
        <Link
          to="/login"
          className="bg-white text-black px-5 py-1.5 rounded-full font-bold text-sm hover:scale-105 transition"
        >
          Iniciar Sesión
        </Link>
      )}
