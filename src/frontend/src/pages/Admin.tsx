...
const ArtistManager = () => {
  ...
  const handleDelete = async (id: number) => {
    await fetch(`http://localhost:3000/artistas/${id}`, { method: "DELETE" });
    fetchArtists();
  };
  ...
  return (
    <div>
      <form onSubmit={handleCreate} className="flex gap-2 mb-6">
        <input
          type="text" placeholder="Nombre del artista" value={nombre}
          onChange={(e) => setNombre(e.target.value)}
          className="flex-1 p-3 rounded bg-zinc-800 text-white border border-zinc-700 focus:outline-none focus:border-green-500"
          required
        />
        <button type="submit" className="bg-green-500 text-black px-6 py-3 rounded-full font-bold hover:scale-105 transition">
          Agregar
        </button>
      </form>
      ...
      <div className="space-y-2">
        {artists.map((a) => (
          <div key={a.id} className="flex items-center justify-between bg-zinc-800/40 p-3 rounded-lg border border-zinc-800">
            ...
            <button onClick={() => handleDelete(a.id)} className="text-red-500 hover:text-red-400 text-sm">
              Eliminar
            </button>
          </div>
        ))}
...
const AlbumManager = () => {
  ...
  const handleDelete = async (id: number) => {
    await fetch(`http://localhost:3000/albumes/${id}`, { method: "DELETE" });
    fetchData();
  };
      ...
      <div className="space-y-2">
        {albums.map((al) => (
          <div key={al.id} className="flex items-center justify-between bg-zinc-800/40 p-3 rounded-lg border border-zinc-800">
            ...
            <button onClick={() => handleDelete(al.id)} className="text-red-500 hover:text-red-400 text-sm">
              Eliminar
            </button>
          </div>
        ))}
...
const SongManager = () => {
  ...
  const handleDelete = async (id: number) => {
    await fetch(`http://localhost:3000/canciones/${id}`, { method: "DELETE" });
    fetchData();
  };
      ...
      <div className="space-y-2">
        {songs.map((s) => (
          <div key={s.id} className="flex items-center justify-between bg-zinc-800/40 p-3 rounded-lg border border-zinc-800">
            ...
            <button onClick={() => handleDelete(s.id)} className="text-red-500 hover:text-red-400 text-sm">
              Eliminar
            </button>
          </div>
        ))}
